...
# Otherwise, use FuzzifierDetermination(x.stand, wv, stand=TRUE) which means the x.stand has been normalized.
summary(FD)
```
...
if(library('cowplot', logical.return = TRUE)){
  cowplot::plot_grid(plotlist = p.spec$p.group.spec.2)
}else{
  message("Recommend you to install 'cowplot' package by running: `install.packages('cowplot')`")
}
```
