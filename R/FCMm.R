...
#'     \item \strong{centroids} The centroids of the cluster result by aggregating each mean.
...
FCM.new <- function(FDlist, K, plot.jitter=TRUE, fast.mode=FALSE,stand=FALSE){
  ...
  centroids <- aggregate(x, list(res$cluster), mean)
...
#' @importFrom reshape2 melt
#' @importFrom magrittr %>% %<>%
plot_spec <- function(res, show.stand = NULL, show.ribbon = FALSE){
  
  if(is.null(show.stand)) show.stand = !res$FD$stand

  p.all.spec <- .plot.all.spec(res, show.stand = show.stand)
  p.cluster.spec <- .plot.cluster.spec(res, show.stand = show.stand, show.ribbon = show.ribbon)
  p.group.spec.1 <- .plot.group.spec(res, show.stand = show.stand, mem.crisp = TRUE)
  p.group.spec.2 <- .plot.group.spec(res, show.stand = show.stand, mem.crisp = FALSE)
...
#' @noRd
#' 
.plot.cluster.spec <- function(res, show.stand = NULL, show.ribbon = FALSE){
  ...
  Rrs.cluster.mean <- aggregate(Rrs_, list(res$res.FCM$cluster), mean) %>% melt(., id = 1)
  Rrs.cluster.sd <- aggregate(Rrs_, list(res$res.FCM$cluster), sd) %>% melt(., id = 1) 
  ...
  if(show.ribbon){
    ...
    result <- ggplot() +
      geom_ribbon(data = Rrs.cluster,
                  ...
                  color = NA, show.legend = FALSE, alpha = 0.1) + 
      geom_path(data = Rrs.cluster,
                aes(x = band, y = mean, group = cluster, color = cluster), 
                size=1.1, alpha=1.5) +
      scale_color_manual(values = RdYlBu(res$K), breaks = seq(1,res$K)) +
      scale_fill_manual(values = RdYlBu(res$K), breaks = seq(1,res$K)) +
...
#' @noRd
#' 
.plot.group.spec <- function(res, show.stand = NULL, mem.crisp=TRUE, mem.threshold=NULL){
  ...
  Rrs.number <- aggregate(Rrs_[,1], list(res$res.FCM$cluster), length) %>%
    setNames(., c("cluster","number"))
  
  Rrs.cluster <- aggregate(Rrs_, list(res$res.FCM$cluster), mean) %>% 
  ...
  if(mem.crisp==TRUE){
    ...
    p <- ggplot() + 
      geom_path(data = Rrs.group, 
                aes(x = band, y = value, group = ids) , color = "gray50", alpha=1, size = 0.5)+
      geom_path(data = Rrs.group_, 
                aes(x = band, y = value, group = ids) , color = "seagreen", alpha=0.3, size = 0.5)+
      geom_path(data = Rrs.cluster, 
                aes(x=band, y=value, group= cluster, color = cluster), alpha = 1.5, size=1.1) + 
      geom_text(data = Rrs.number,
                aes(x=-Inf, y=Inf, label = sprintf("N=%s",number)), vjust=1.2, hjust = -0.2) + 
      scale_color_manual(values = RdYlBu(res$K), breaks = seq(1,res$K)) +
      facet_wrap(~as.numeric(cluster)) + 
